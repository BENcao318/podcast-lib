      ...
      <Popover className="relative">
        {({ open, close }) => (
          <>
            ...
            >
              <Popover.Panel className="absolute left-26 z-10 mt-3 w-screen max-w-sm translate-x-48 -translate-y-14 ansform px-4 sm:px-0 lg:max-w-3xl">
                <div className="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                  <div className="relative grid gap-6 bg-white p-6 lg:grid-cols-2">
                    {genres.map((item) => (
                      <div
                        key={item.name}
                        className="-m-2 flex items-center rounded-lg p-2 transition duration-150 ease-in-out hover:bg-sky-400 focus:outline-none focus-visible:ring focus-visible:ring-orange-500 focus-visible:ring-opacity-50"
                        onClick={() => {
                          close()
